...
setwd("/Users/mattdennis/Documents/DocumentsMatt/floodplain/")
...
damBufferRiv$damIDs<-1:nrow(damBufferRiv)#need this to track IDs later
...
precrec_proc <- evalmod(scores = pred,labels = dataTest$pres,mode = "prcroc")

plot(precrec_proc)

modauc <- precrec::auc(precrec::evalmod(scores = pred, 
                                        labels = dataTest$pres))
modauc
